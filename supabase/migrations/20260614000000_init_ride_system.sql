...
begin
    ...
    if not exists (select 1 from pg_policies where policyname = 'Admins can read all rides' and tablename = 'rides') then
        create policy "Admins can read all rides" on public.rides for select using (exists (select 1 from public.profiles where id = auth.uid() and role = 'admin'));
    end if;
...
returns void as $$
begin
  update public.rides set status = p_new_status, updated_at = now(), started_at = case when p_new_status = 'in_progress' then now() else started_at end, completed_at = case when p_new_status = 'completed' then now() else completed_at end where id = p_ride_id and (driver_id in (select id from public.drivers where user_id = auth.uid()) or exists (select 1 from public.profiles where id = auth.uid() and role = 'admin'));
  if not found then
...
returns void as $$
begin
  update public.rides set status = 'cancelled', cancelled_at = now(), updated_at = now(), cancellation_reason = p_reason where id = p_ride_id and status in ('requested', 'accepted', 'driver_arriving') and (customer_id = auth.uid() or driver_id in (select id from public.drivers where user_id = auth.uid()) or exists (select 1 from public.profiles where id = auth.uid() and role = 'admin'));
  if not found then
