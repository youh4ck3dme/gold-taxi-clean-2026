...
class MockApiService extends ApiService {
  ...
  static final List<String> _streets = [
    'Hlavná',
    'Nová',
    'Stará',
    'Lesná',
  ...
  static final List<String> _providers = [
    'Gold-Taxi',
    'Taxi Bratislava',
    'City Taxi',
    'Euro Taxi',
  ...
  MockApiService(super.authInterceptor) : super(enableMockMode: true);
  ...
  static List<Map<String, dynamic>> getMockCptData(
    String cptName, {
    int count = 10,
  }) {
    ...
    if (normEndpoint.contains('/wp-json/wp/v2/media')) {
      return List.generate(
        ...
        (i) => {
          'id': 100 + i,
          'source_url':
              'https://via.placeholder.com/800x600/FFD700/000000?text=Image+${100 + i}',
  ...
  static List<Map<String, dynamic>> getMockPosts({int count = 10}) {
    return List.generate(
      ...
      (index) => {
        ...
        'title': {
          'rendered':
              'Ako objednať taxi cez Gold-Taxi: Príručka pre začičínajúcich',
        },
        'content': {
          'rendered':
              'Vítame vás v Gold-Taxi! Objednajte si taxi jednoducho cez našu aplikáciu...',
        },
        'excerpt': {'rendered': 'Návod na používanie aplikácie Gold-Taxi'},
          ...
          'wp:featuredmedia': [
            {
              'id': 100 + index,
              'source_url':
                  'https://via.placeholder.com/800x400/FFD700/000000?text=Blog+${index + 1}',
            },
          ],
        },
      },
  ...
  static List<Map<String, dynamic>> getMockProducts({int count = 6}) {
    return List.generate(
      ...
      (index) => {
        ...
        'description':
            'Taxi služba ${_services[index % _services.length]} - komfortné a rýchle prepravenie',
        ...
        'images': [
          {
            'id': 200 + index,
            'src':
                'https://via.placeholder.com/400x300/FFD700/000000?text=Taxi+${_services[index % _services.length]}',
          },
        ],
        'categories': [
          {'id': 1, 'name': 'Taxi Služby', 'slug': 'taxi-sluzby'},
        ...
        'meta_data': [
          {
            'id': 1,
            'key': 'duration',
            'value': '${_random.nextInt(30) + 10} min',
          },
        ...
        'button_text': '',
        'price_html':
            '<span class="woocommerce-Price-amount amount"><bdi>€${_randomPrice().toStringAsFixed(2)}</bdi></span>',
  ...
  static List<Map<String, dynamic>> getMockUsers({int count = 10}) {
    return List.generate(
      ...
      (index) => {
        ...
        'email':
            '${_firstNames[index % _firstNames.length].toLowerCase()}.${_lastNames[index % _lastNames.length].toLowerCase()}@goldtaxi.sk',
        ...
        'acf': {
          ...
          'car_plate':
              '${String.fromCharCode(65 + _random.nextInt(26))}${String.fromCharCode(65 + _random.nextInt(26))} ${_random.nextInt(900) + 100}',
  ...
  static List<Map<String, dynamic>> getMockBookings({int count = 8}) {
    return List.generate(
      ...
      (index) => {
        ...
        'pickup_location':
            '${_streets[index % _streets.length]} ${_random.nextInt(100) + 1}, ${_cities[index % _cities.length]}',
        'dropoff_location':
            '${_streets[_random.nextInt(_streets.length)]} ${_random.nextInt(100) + 1}, ${_cities[_random.nextInt(_cities.length)]}',
        'pickup_date': _randomFutureDate().toIso8601String(),
        'dropoff_date': _randomFutureDate()
            .add(Duration(minutes: _random.nextInt(60) + 15))
            .toIso8601String(),
        ...
        'rating': _random.nextBool()
            ? _randomRating().toStringAsFixed(1)
            : null,
    ...
    return List.generate(
      ...
      (index) => {
        ...
        'author_name':
            '${_firstNames[index % _firstNames.length]} ${_lastNames[index % _lastNames.length]}',
        'author_email':
            '${_firstNames[index % _firstNames.length].toLowerCase()}.${_lastNames[index % _lastNames.length].toLowerCase()}@example.com',
    ...
    return List.generate(
      ...
      (index) => {
        ...
        'items': List.generate(
          ...
          (i) => {
            'description':
                'Taxi služba ${_services[_random.nextInt(_services.length)]}',
            'quantity': 1,
  ...
  static List<Map<String, dynamic>> getMockServices({int count = 6}) {
    return List.generate(
      ...
      (index) => {
        ...
        'name': _services[index % _services.length],
        'description':
            'Profesionálna taxi služba ${_services[index % _services.length]} pre vaše pohodlie',
        'price': (_randomPrice() * 10).toStringAsFixed(2),
        'category': 'Taxi',
        ...
        'images': [
          'https://via.placeholder.com/400x300/FFD700/000000?text=${_services[index % _services.length]}',
        ],
  ...
  static List<Map<String, dynamic>> getMockEvents({int count = 4}) {
    return List.generate(
      ...
      (index) => {
        ...
        'content': {
          'rendered':
              'Pridajte sa k nám a využite výhody! Akcia platí do 31.12.2026',
        },
        'excerpt': {
          'rendered': const [
        ...
        'end_date': _randomFutureDate()
            .add(const Duration(days: 7))
            .toIso8601String(),
        'location': _cities[index % _cities.length],
        'organizer': 'Gold-Taxi',
        'featured_image':
            'https://via.placeholder.com/800x400/FFD700/000000?text=Event+${index + 1}',
        'status': 'publish',
      },
    );
  }
      ...
      'addresses': [
        {
          ...
          'label': 'Domov',
          'address':
              '${_streets[_random.nextInt(_streets.length)]} ${_random.nextInt(50) + 1}, ${_cities[_random.nextInt(_cities.length)]}',
          'lat': 48.1486,
          'lng': 17.1077,
          'is_default': true,
        },
