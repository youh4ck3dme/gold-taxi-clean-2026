...
class DriverProfileService {
  ...
  static const String tableName = 'driver_profiles';
  ...
  Future<DriverPositionModel> createOrUpdateDriverProfile(
    ...
    try {
      ...
      // Add driverId to the map (it's used as the row ID in Supabase)
      driverData['id'] = driver.driverId;
      ...
      final savedDriver = DriverPositionModel.fromMap(
        driver.driverId,
        data,
      );
  ...
  Future<DriverPositionModel> createDriverProfile(
    ...
    try {
      ...
      final createdDriver = DriverPositionModel.fromMap(
        driver.driverId,
        data,
      );

      _logger.i('✅ Driver profile created successfully: ${createdDriver.driverId}');
  ...
  Future<DriverPositionModel?> getDriverProfile(String driverId) async {
    try {
      ...
      final driver = DriverPositionModel.fromMap(
        driverId,
        data,
      );
  ...
  Future<List<DriverPositionModel>> getAllDriverProfiles() async {
    try {
      ...
      for (final data in dataList) {
        final driverId = data['id'] as String? ?? data['driverId'] as String? ?? '';
  ...
  Future<DriverPositionModel> updateDriverPosition({
    ...
    try {
      ...
      final updatedDriver = DriverPositionModel.fromMap(
        driverId,
        data,
      );
