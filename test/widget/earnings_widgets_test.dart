...
void main() {
  group('BankAccountDialog Widget Tests', () {
    testWidgets('1. BankAccountDialog renders fields and handles save', (
      WidgetTester tester,
    ) async {
      ...
      // Find TextFormFields
      final stripeField = find.ancestor(
        of: find.text('Stripe Account ID'),
        matching: find.byType(TextFormField),
      );
      final bankField = find.ancestor(
        of: find.text('Názov banky'),
        matching: find.byType(TextFormField),
      );
      final holderField = find.ancestor(
        of: find.text('Meno majiteľa účtu'),
        matching: find.byType(TextFormField),
      );
      final digitsField = find.ancestor(
        of: find.text('Posledné 4 číslice účtu / IBANu'),
        matching: find.byType(TextFormField),
      );
    ...
    testWidgets('2. BankAccountDialog validation works', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
    ...
    testWidgets('3. BankAccountDialog invalid Stripe ID validation works', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        MaterialApp(
      ...
      expect(
        find.text('Stripe Account ID musí začínať s "acct_"'),
        findsOneWidget,
      );
    });
  });
