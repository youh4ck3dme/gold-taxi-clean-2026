...
void main() {
  ...
  group('ChatCubit Tests', () {
    ...
    test(
      'initChat subscribes to messages stream and emits ChatLoaded',
      () async {
        final controller = StreamController<List<ChatMessageModel>>();
        when(
          () => mockChatRepository.getMessagesStream('ride_123'),
    ...
    test('initChat emits ChatError on stream error', () async {
      ...
      expectLater(
        chatCubit.stream,
        emitsInOrder([isA<ChatLoading>(), isA<ChatError>()]),
      );
    ...
    test(
      'sendMessage calls repository and does not emit error on success',
      () async {
        when(
    ...
    test('initiateMaskedCall sets isCalling true then false', () async {
      ...
      when(
        () => mockChatRepository.triggerMaskedCall(
          rideId: any(named: 'rideId'),
      ...
      expectLater(
        ...
        emitsInOrder([
          predicate<ChatState>(
            (state) => state is ChatLoaded && state.isCalling == true,
          ),
          predicate<ChatState>(
            (state) => state is ChatLoaded && state.isCalling == false,
          ),
        ]),
      );
