...
void main() {
  group('InsolvencyPredictorService Tests', () {
    ...
    test('Dobrá platobná morálka vyhodnotí nízke riziko', () {
      final invoices = [
        InvoiceModel(
          ...
          paidDate: now.subtract(const Duration(days: 139)), // 1 deň oneskorenie
        ),
        InvoiceModel(
          id: 'FA-2',
          amount: 1200.0,
      ...
      final prediction = predictorService.analyzeInsolvencyRisk(invoices, evaluationDate: now);
    ...
    test('Oneskorené platby a veľa otvorených faktúr vyhodnotí vysoké riziko úpadku', () {
      final invoices = [
        InvoiceModel(
          id: 'FA-11',
          amount: 1000.0,
          issueDate: now.subtract(const Duration(days: 150)),
          dueDate: now.subtract(const Duration(days: 140)),
          paidDate: now.subtract(const Duration(days: 100)), // 40 dní oneskorenie
        ),
        InvoiceModel(
          id: 'FA-12',
          amount: 2000.0,
      ...
      expect(prediction.riskFactors, contains(contains('faktúry viac ako 90 dní po splatnosti')));
    });

    test('Prázdny zoznam faktúr vráti nízke riziko s vysvetlením', () {
      final prediction = predictorService.analyzeInsolvencyRisk([], evaluationDate: now);

      expect(prediction.riskLevel, 'Nízke');
      expect(prediction.riskScore, 0.0);
      expect(prediction.predictsInsolvencyIn3Months, isFalse);
      expect(prediction.riskFactors.first, contains('Nedostatok platobných údajov'));
    });
  });
}
