...
void main() {
  ...
  group('BookingsBloc Tests', () {
    ...
    test('FetchAvailableSlots emits [BookingsLoading, BookingsSlotsLoaded] when repository succeeds', () async {
      when(() => mockBookingsRepository.getAvailableSlots(any(), any()))
      ...
      expectLater(
        ...
        emitsInOrder([
          isA<BookingsLoading>(),
          isA<BookingsSlotsLoaded>(),
        ]),
      );

      bookingsBloc.add(const FetchAvailableSlots(serviceId: 2, date: '2026-06-12'));
    });

    test('FetchAvailableSlots emits [BookingsLoading, BookingsError] when repository throws error', () async {
      when(() => mockBookingsRepository.getAvailableSlots(any(), any()))
    ...
    test('SubmitBooking emits [BookingSubmissionInProgress, BookingSubmissionSuccess] when repository succeeds', () async {
      when(() => mockBookingsRepository.bookSlot(
      ...
      bookingsBloc.add(const SubmitBooking(
        serviceId: 2,
        date: '2026-06-12',
        timeSlot: '14:00',
      ));
    ...
    test('SubmitBooking emits [BookingSubmissionInProgress, BookingSubmissionFailure] when repository fails', () async {
      when(() => mockBookingsRepository.bookSlot(
            serviceId: any(named: 'serviceId'),
            date: any(named: 'date'),
            timeSlot: any(named: 'timeSlot'),
