...
void main() {
  group('Supabase Filter Regression Tests - Rides Repository', () {
    ...
    test('getAllRides() uses semantic status filter with RLS documentation', () async {
      final filePath = path.join('lib', 'features', 'map', 'data', 'repositories', 'supabase_ride_repository.dart');
      final source = await File(filePath).readAsString();
      
      // Verify semantic filter exists
      expect(source.contains('getAllRides()'), isTrue);
      expect(source.contains('.neq(\'status\''), isTrue,
          reason: 'Admin ride stream must use semantic status filter .neq()');
      
      // Verify RLS documentation exists
      expect(source.contains('RLS policy:'), isTrue,
          reason: 'Semantic filter must be documented with RLS policy reference');
      expect(source.contains('Users can see involved rides'), isTrue,
      ...
      expect(hasNeqId, isFalse,
          reason: 'getAllRides() must NOT use .neq(\'id\') - uses .neq(\'status\') instead');
    });
    ...
    test('All rides SELECT queries have filters', () async {
      ...
      final filterMethods = RegExp(r'\.(eq|neq|gt|lt|gte|lte|in_|filter)\(').allMatches(source);
      
      expect(fromRidesMatches.length <= filterMethods.length, isTrue,
          reason: 'Every .from(\'rides\') must have a filter. '
  ...
  group('Supabase Filter Regression Tests - FAQ Repository', () {
    ...
    test('getFaqs() has documented temporary filter', () async {
      final filePath = path.join('lib', 'features', 'faq', 'data', 'repositories', 'faq_repository.dart');
      final source = await File(filePath).readAsString();
      ...
      expect(source.contains('TODO: Consider adding is_active boolean field'), isTrue,
    ...
    test('No unfiltered faq SELECT queries exist', () async {
      ...
      expect(fromFaqsMatches.length <= filterMethods.length, isTrue,
  ...
  group('Production Hardening - Environment Validation', () {
    ...
    test('Production main_prod.dart has NO default Supabase URL', () async {
      ...
      expect(source.contains("fromEnvironment('SUPABASE_URL', defaultValue:"), isFalse,
          reason: 'Production must NOT have defaultValue for SUPABASE_URL');
      
      // Must NOT hardcode the URL
      expect(source.contains('nscxuxhapaabtsiduxlu.supabase.co'), isFalse,
          reason: 'Production must NOT hardcode Supabase project URL');
    });

    test('Production main_prod.dart has NO default Supabase anon key', () async {
      final filePath = path.join('lib', 'main_prod.dart');
      final source = await File(filePath).readAsString();
      ...
      expect(source.contains("fromEnvironment('SUPABASE_ANON_KEY', defaultValue:"), isFalse,
          reason: 'Production must NOT have defaultValue for SUPABASE_ANON_KEY');
      
      // Check for any hardcoded JWT-like strings
      expect(source.contains('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9'), isFalse,
    ...
    test('Production main_prod.dart throws on MOCK_MODE=true', () async {
      ...
      expect(source.contains('MOCK_MODE'), isTrue,
          reason: 'Production must check MOCK_MODE');
      
      // Must assert it's false
      expect(source.contains('MOCK_MODE must be false in production builds'), isTrue,
          reason: 'Production must assert MOCK_MODE is false');
      
      // Must throw on violation
      expect(source.contains("throw StateError('Security violation:"), isTrue,
          reason: 'Production must throw StateError on MOCK_MODE security violation');
    });
    ...
    test('Production requires dart defines for Supabase config', () async {
      ...
      final envVars = RegExp(r"fromEnvironment\('([^']+)'\)").allMatches(source);
      expect(envVars.length >= 3, isTrue,
  ...
  group('Supabase Filter Regression Tests - Edge Functions', () {
    ...
    test('Edge functions have Prefer header for SELECT queries', () async {
      ...
      final twilioPath = path.join('supabase', 'functions', 'twilio_masked_call', 'index.ts');
      if (await File(twilioPath).exists()) {
        final source = await File(twilioPath).readAsString();
        expect(source.contains("'Prefer': 'return=representation'"), isTrue,
            reason: 'twilio_masked_call must have Prefer header');
      }
      
      // Check stripe_webhook
      final stripePath = path.join('supabase', 'functions', 'stripe_webhook', 'index.ts');
