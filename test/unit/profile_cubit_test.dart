...
void main() {
  ...
  group('ProfileCubit Tests', () {
    ...
    test(
      'fetchProfile emits [ProfileLoading, ProfileLoaded] for customer',
      () async {
        when(
          () => mockProfileRepository.getUserProfile(),
        ).thenAnswer((_) async => testCustomer);
        when(
          () => mockProfileRepository.getOrderHistory(any()),
        ).thenAnswer((_) async => []);
        when(
          () => mockProfileRepository.getBookingHistory(any()),
        ).thenAnswer((_) async => []);
    ...
    test(
      'fetchProfile emits [ProfileLoading, ProfileLoaded] with driverStats for driver',
      () async {
        when(
          () => mockProfileRepository.getUserProfile(),
        ...
        when(() => mockProfileRepository.getDriverRecord(any())).thenAnswer(
          (_) async => {
            'id': 'driver-uuid-123',
            'is_online': true,
        ...
        when(() => mockProfileRepository.getDriverStats(any())).thenAnswer(
          (_) async => {
            'totalRides': 42,
        ...
        when(
          () => mockProfileRepository.getDriverDocuments(any()),
        ...
        verify(
          () => mockProfileRepository.getDriverStats('driver-uuid-123'),
        ).called(1);
      },
    );

    test(
      'fetchProfile works when driver has no completed rides (empty stats)',
      () async {
        when(
          () => mockProfileRepository.getUserProfile(),
        ).thenAnswer((_) async => testDriver);
        ...
        when(() => mockProfileRepository.getDriverRecord(any())).thenAnswer(
          (_) async => {'id': 'driver-uuid-456', 'is_online': false},
        );
        when(() => mockProfileRepository.getDriverStats(any())).thenAnswer(
          (_) async => {
            'totalRides': 0,
    ...
    test(
      'fetchProfile emits [ProfileLoading, ProfileError] when repository fails',
      () async {
        when(
          () => mockProfileRepository.getUserProfile(),
        ...
        expectLater(
          profileCubit.stream,
          emitsInOrder([isA<ProfileLoading>(), isA<ProfileError>()]),
        );
    ...
    test(
      'updateCustomerProfile emits [ProfileUpdating, ProfileLoaded] on successful update',
      () async {
        // Setup initial state as loaded first
        profileCubit.emit(
          const ProfileLoaded(user: testCustomer, orders: [], bookings: []),
        );

        when(
          () => mockProfileRepository.updateCustomerProfile(
        ...
        expectLater(
          profileCubit.stream,
          emitsInOrder([isA<ProfileUpdating>(), isA<ProfileLoaded>()]),
        );
    ...
    test(
      'updateDriverProfile preserves driverStats in emitted state',
      () async {
        final initialStats = {
          'totalRides': 10,
          'totalEarnings': 500.0,
        ...
        when(
          () => mockProfileRepository.updateDriverProfile(
        ...
        when(
          () => mockProfileRepository.getDriverRecord(any()),
        ).thenAnswer((_) async => {'id': 'driver-1', 'is_online': true});
    ...
    test(
      'registerAsDriver emits [ProfileUpdating, ProfileLoaded] and calls registerAsDriver',
      () async {
        profileCubit.emit(
          const ProfileLoaded(user: testCustomer, orders: [], bookings: []),
        );
        ...
        when(
          () => mockProfileRepository.registerAsDriver(
            vehicleType: any(named: 'vehicleType'),
            vehiclePlate: any(named: 'vehiclePlate'),
            serviceClasses: any(named: 'serviceClasses'),
          ),
    ...
    test('uploadDriverDocuments uploads 3 docs and saves references', () async {
      profileCubit.emit(
        const ProfileLoaded(user: testDriver, orders: [], bookings: []),
      );

      when(
        () => mockProfileRepository.uploadDocument(
      ...
      when(
        () => mockProfileRepository.saveDriverDocuments(
      ...
      when(
        () => mockProfileRepository.getDriverRecord(any()),
      ).thenAnswer((_) async => {'id': 'driver_test'});
      when(
        () => mockProfileRepository.getDriverStats(any()),
      ).thenAnswer((_) async => {});
      when(
        () => mockProfileRepository.getDriverDocuments(any()),
      ).thenAnswer((_) async => {});
      ...
      verify(
        () => mockProfileRepository.saveDriverDocuments(
          profilePhotoUrl:
              'https://supabase.storage/driver-documents/profile_photo.jpg',
    ...
    test('sendPhoneOtp calls sendPhoneOtp on repository', () async {
      when(
        () => mockProfileRepository.sendPhoneOtp(any()),
      ).thenAnswer((_) async => {});

      await profileCubit.sendPhoneOtp('+421900123456');

      verify(
        () => mockProfileRepository.sendPhoneOtp('+421900123456'),
      ).called(1);
    });

    test(
      'verifyPhoneOtp returns true on success and false on failure',
      () async {
        when(
          () => mockProfileRepository.verifyPhoneOtp('+421900123456', '123456'),
        ).thenAnswer((_) async => true);
        when(
          () => mockProfileRepository.verifyPhoneOtp('+421900123456', '000000'),
        ).thenAnswer((_) async => false);

        final success = await profileCubit.verifyPhoneOtp(
          '+421900123456',
          '123456',
        );
        final failure = await profileCubit.verifyPhoneOtp(
          '+421900123456',
          '000000',
        );
    ...
    test(
      'applyReferralCode calls applyReferralCode and fetches profile on success',
      () async {
        profileCubit.emit(
          const ProfileLoaded(user: testCustomer, orders: [], bookings: []),
        );
        ...
        when(
          () => mockProfileRepository.applyReferralCode('MICHAL80'),
        ).thenAnswer((_) async => {});
        when(
          () => mockProfileRepository.getUserProfile(),
        ).thenAnswer((_) async => testCustomer);
    ...
    test('applyReferralCode returns error message on exception', () async {
      ...
      when(
        () => mockProfileRepository.applyReferralCode('INVALID'),
      ).thenThrow(Exception('Neplatný referenčný kód.'));
