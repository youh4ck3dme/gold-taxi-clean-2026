...
void main() {
  group('Production Hardening Regression Tests', () {
    ...
    test('2. ApiService must not contain mock manufacturing code or fallback logic', () {
      ...
      for (final term in prohibited) {
        expect(content.contains(term), isFalse, reason: 'ApiService must not contain $term');
      }
    });

    test('3. ApiService Dio connection timeout throws ApiException with timeout message (no mock fallback)', () async {
      ...
      expect(
        ...
        throwsA(isA<ApiException>().having((e) => e.message, 'message', contains('Connection timeout'))),
      );
    });

    test('4. EarningsCubit throws EarningsError immediately when driverId is empty', () async {
      final mockRepo = MockEarningsRepository();
      final cubit = EarningsCubit(earningsRepository: mockRepo, driverId: '');
      
      expectLater(
        cubit.stream,
        emits(isA<EarningsError>().having(
          (e) => e.message,
          'message',
