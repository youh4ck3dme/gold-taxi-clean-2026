...
void main() {
  ...
  group('CPT Routing & Caching Tests', () {
    test('WP CPT is available: fetches directly from /wp-json/wp/v2/booking', () async {
    ...
    test('WP CPT 404s, but JetEngine is available: fetches from /wp-json/jet-engine/v1/booking', () async {
      ...
      when(() => mockDio.get(
        ...
        requestOptions: RequestOptions(path: 'wp-json/jet-engine/v1/booking'),
        statusCode: 200,
      ));
      ...
      verify(() => mockDio.get('wp-json/wp/v2/booking', queryParameters: {'per_page': 1})).called(1);
      verify(() => mockDio.get('wp-json/jet-engine/v1/booking', queryParameters: {'per_page': 1})).called(1);
    });

    test('All CPT routes 404: falls back to /wp-json/wp/v2/posts?type=booking', () async {
      ...
      when(() => mockDio.get(
        'wp-json/wp/v2/posts',
        queryParameters: {'page': 1, 'per_page': 10, '_embed': 1, 'type': 'booking'},
        options: any(named: 'options'),
      )).thenAnswer((_) async => Response(
    ...
    test('Endpoint caching: probe check is only called once per route', () async {
      when(() => mockDio.get(
        'wp-json/wp/v2/booking',
        queryParameters: {'per_page': 1},
      )).thenAnswer((_) async => Response(
  ...
  group('Model Deserialization Tests', () {
    test('FaqModel parsing resiliently maps fields from custom field or wp post title/content', () {
      final faqJson = {
        'id': 1,
