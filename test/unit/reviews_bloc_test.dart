...
void main() {
  ...
  group('ReviewsBloc Tests', () {
    ...
    test(
      'FetchReviews emits [ReviewsLoading, ReviewsLoaded] when repository succeeds',
      () async {
        when(
          () => mockReviewsRepository.getReviews(any()),
    ...
    test(
      'FetchReviews emits [ReviewsLoading, ReviewsError] when repository throws error',
      () async {
        when(
          () => mockReviewsRepository.getReviews(any()),
        ...
        expectLater(
          reviewsBloc.stream,
          emitsInOrder([isA<ReviewsLoading>(), isA<ReviewsError>()]),
        );
    ...
    test(
      'SubmitReview emits [ReviewSubmissionInProgress, ReviewSubmissionSuccess] when repository succeeds',
      () async {
        when(
          () => mockReviewsRepository.submitReview(
    ...
    test(
      'SubmitReview emits [ReviewSubmissionInProgress, ReviewSubmissionFailure] when repository fails',
      () async {
        when(
          () => mockReviewsRepository.submitReview(
            postId: any(named: 'postId'),
